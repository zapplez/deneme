...
class DatasetHandler(object):
    ...
    def __gen_ds(self, paths, dataset_name, dataset, train_transform, test_transform, split='train', hp_filtered=False):
        if dataset_name == 'cityscapes':
            ...
            elif self.args.client_data_type == 'RGB&D':
                if split=="train":
    ...
    def __target_dataset_init(self):
        ...
        for users, split_data, split in zip((train_users, test_users), (train_data, test_data), ('train', 'test')):
            ...
            for i, user in enumerate(users):
                ...
                if user=="test_user" and (self.args.client_data_type == 'RGB||D' and self.args.num_encoders == self.args.num_decoders) or user=="test_user" and (self.args.client_data_type == 'RGB||D' and self.args.num_encoders != self.args.num_decoders):
        ...
        if self.args.framework == 'federated' and (self.args.client_data_type == 'RGB||D' and self.args.num_encoders == self.args.num_decoders)  or self.args.framework == 'federated' and (self.args.client_data_type == 'RGB||D' and self.args.num_encoders != self.args.num_decoders):
        ...
        if (self.args.framework == 'federated'and self.args.client_data_type == 'RGB&D') or (self.args.framework == 'federated'and self.args.client_data_type == 'RGB'):
            ds = self.__gen_ds(all_train_data['centralized_user'],
                               dataset_name, dataset, train_transform, test_transform, split='test')
            self.clients_args['all_train'].append({'client_id': 'all_target_train_data', 'dataset': ds})
